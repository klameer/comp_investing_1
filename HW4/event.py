...
def find_events(ls_symbols, d_data, eventAmount):
    ...
    for s_sym in ls_symbols:
        for i in range(1, len(ldt_timestamps)):
            ...
            if f_symprice_yest >= eventAmount and f_symprice_today < eventAmount:
...
def create_orders(ldt_timestamps, symbols_list, eventAmount):
    dataobj = da.DataAccess('Yahoo')
    ls_symbols = dataobj.get_symbols_from_list(symbols_list)
    ls_symbols.append('SPY')
    ...
    df_events = find_events(ls_symbols, d_data, eventAmount)

    file_out = open( ordersFile, "w" )
